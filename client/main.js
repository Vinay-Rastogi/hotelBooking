document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function submitGasBookingForm() {
    ...
    var selectedRadio = document.querySelector('input[name="roomType"]:checked');
    var roomType = selectedRadio ? selectedRadio.value : null;


    if (!bookedDate || !days || !roomType) {
    ...
    try {
      const response = await fetch(`/book-gas`, {
        ...
        body: JSON.stringify({ bookedDate, days, roomType }),
      });
  ...
  async function editBookingAddress() {
    const updatedAddress = document.getElementById("Date").value;

    if (!updatedAddress) {
      displayErrorMessage("Please enter the new booked date.");
      return;
    }
        ...
        if (allBookings.length <= 0) {
          ...
          allBookings.forEach((booking, index) => {
            ...
            listItem.innerHTML = `
                                ...
                                <p><strong>Room Booked for Date:</strong> <span class="address">${
                                  booking.bookedDate
                                }</span></p>
                                <p><strong>Number of days:</strong> <span class="address">${
                                  booking.days
                                }</span></p>
                                <p><strong>Room Type:</strong> <span class="booking-date">${
                                  booking.roomType}</span></p>
                                <hr>
                            `;
